package com.xxl.job.executor.service.jobhandler.model;

import com.thoughtworks.xstream.annotations.XStreamAlias;

/**
 * @program: xxl-job
 * @author: xi.chen
 * @create: 2019-08-29 15:04
 **/
@XStreamAlias("dataNode")
public class MedInstruYlqxjyxk implements java.io.Serializable{

    private String QYMC="";
    private String ZZJGDM="";
    private String JYFS="";
    private String ZS="";
    private String JYCS="";
    private String CKDZ="";
    private String FRDB="";
    private String QYFZRXM="";
    private String ZLFZRXM="";
    private String JYFW="";
    private String XKZBH="";
    private String FZRQ="";
    private String ZJZSYXQZZRQ="";
    private String FZJG="";
    private String QYSZS="";
    private String QYSZSHI="";
    private String QYSZX="";
    private String PPZT="";
    private String BGLSJL="";
    private String BZ="";
    private String LXR="";
    private String LXDH="";
    private String JYCSMJ="";
    private String CKMJ="";
    private String ZGZS="";
    private String ZYJSRS="";
    private String YXKZBH="";
    private String ZTBZYY="";
    private String ZTBZSJ="";

    public String getQYMC() {
        return QYMC;
    }

    public void setQYMC(String QYMC) {
        this.QYMC = QYMC;
    }

    public String getZZJGDM() {
        return ZZJGDM;
    }

    public void setZZJGDM(String ZZJGDM) {
        this.ZZJGDM = ZZJGDM;
    }

    public String getJYFS() {
        return JYFS;
    }

    public void setJYFS(String JYFS) {
        this.JYFS = JYFS;
    }

    public String getZS() {
        return ZS;
    }

    public void setZS(String ZS) {
        this.ZS = ZS;
    }

    public String getJYCS() {
        return JYCS;
    }

    public void setJYCS(String JYCS) {
        this.JYCS = JYCS;
    }

    public String getCKDZ() {
        return CKDZ;
    }

    public void setCKDZ(String CKDZ) {
        this.CKDZ = CKDZ;
    }

    public String getFRDB() {
        return FRDB;
    }

    public void setFRDB(String FRDB) {
        this.FRDB = FRDB;
    }

    public String getQYFZRXM() {
        return QYFZRXM;
    }

    public void setQYFZRXM(String QYFZRXM) {
        this.QYFZRXM = QYFZRXM;
    }

    public String getZLFZRXM() {
        return ZLFZRXM;
    }

    public void setZLFZRXM(String ZLFZRXM) {
        this.ZLFZRXM = ZLFZRXM;
    }

    public String getJYFW() {
        return JYFW;
    }

    public void setJYFW(String JYFW) {
        this.JYFW = JYFW;
    }

    public String getXKZBH() {
        return XKZBH;
    }

    public void setXKZBH(String XKZBH) {
        this.XKZBH = XKZBH;
    }

    public String getFZRQ() {
        return FZRQ;
    }

    public void setFZRQ(String FZRQ) {
        this.FZRQ = FZRQ;
    }

    public String getZJZSYXQZZRQ() {
        return ZJZSYXQZZRQ;
    }

    public void setZJZSYXQZZRQ(String ZJZSYXQZZRQ) {
        this.ZJZSYXQZZRQ = ZJZSYXQZZRQ;
    }

    public String getFZJG() {
        return FZJG;
    }

    public void setFZJG(String FZJG) {
        this.FZJG = FZJG;
    }

    public String getQYSZS() {
        return QYSZS;
    }

    public void setQYSZS(String QYSZS) {
        this.QYSZS = QYSZS;
    }

    public String getQYSZSHI() {
        return QYSZSHI;
    }

    public void setQYSZSHI(String QYSZSHI) {
        this.QYSZSHI = QYSZSHI;
    }

    public String getQYSZX() {
        return QYSZX;
    }

    public void setQYSZX(String QYSZX) {
        this.QYSZX = QYSZX;
    }

    public String getPPZT() {
        return PPZT;
    }

    public void setPPZT(String PPZT) {
        this.PPZT = PPZT;
    }

    public String getBGLSJL() {
        return BGLSJL;
    }

    public void setBGLSJL(String BGLSJL) {
        this.BGLSJL = BGLSJL;
    }

    public String getBZ() {
        return BZ;
    }

    public void setBZ(String BZ) {
        this.BZ = BZ;
    }

    public String getLXR() {
        return LXR;
    }

    public void setLXR(String LXR) {
        this.LXR = LXR;
    }

    public String getLXDH() {
        return LXDH;
    }

    public void setLXDH(String LXDH) {
        this.LXDH = LXDH;
    }

    public String getJYCSMJ() {
        return JYCSMJ;
    }

    public void setJYCSMJ(String JYCSMJ) {
        this.JYCSMJ = JYCSMJ;
    }

    public String getCKMJ() {
        return CKMJ;
    }

    public void setCKMJ(String CKMJ) {
        this.CKMJ = CKMJ;
    }

    public String getZGZS() {
        return ZGZS;
    }

    public void setZGZS(String ZGZS) {
        this.ZGZS = ZGZS;
    }

    public String getZYJSRS() {
        return ZYJSRS;
    }

    public void setZYJSRS(String ZYJSRS) {
        this.ZYJSRS = ZYJSRS;
    }

    public String getYXKZBH() {
        return YXKZBH;
    }

    public void setYXKZBH(String YXKZBH) {
        this.YXKZBH = YXKZBH;
    }

    public String getZTBZYY() {
        return ZTBZYY;
    }

    public void setZTBZYY(String ZTBZYY) {
        this.ZTBZYY = ZTBZYY;
    }

    public String getZTBZSJ() {
        return ZTBZSJ;
    }

    public void setZTBZSJ(String ZTBZSJ) {
        this.ZTBZSJ = ZTBZSJ;
    }
}