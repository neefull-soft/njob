package com.xxl.job.executor.service.jobhandler.model.nationalShare;

import com.thoughtworks.xstream.annotations.XStreamAlias;

@XStreamAlias("dataNode")
public class NationalShareYpsc implements java.io.Serializable{

    private String ZZJGDM;
    private String XKZBH;
    private String QYMC;
    private String SCDZCLOB;
    private String YPQYSCFW;
    private String ZCDZ;
    private String FRDB;
    private String QYFZRXM;
    private String QYLX;
    private String ZLFZRXM;
    private String QYFLM;
    private String FZJG;
    private String FZRQ;
    private String ZJZSYXQZZRQ;
    private String DZSQZZZM;
    private String YXKZBH;
    private String BGLSJL;
    private String PPZT;
    private String ZTBZYY;
    private String ZTBZSJ;
    private String ID;

    public String getZZJGDM() {
        return ZZJGDM;
    }

    public void setZZJGDM(String ZZJGDM) {
        this.ZZJGDM = ZZJGDM;
    }

    public String getXKZBH() {
        return XKZBH;
    }

    public void setXKZBH(String XKZBH) {
        this.XKZBH = XKZBH;
    }

    public String getQYMC() {
        return QYMC;
    }

    public void setQYMC(String QYMC) {
        this.QYMC = QYMC;
    }

    public String getSCDZCLOB() {
        return SCDZCLOB;
    }

    public void setSCDZCLOB(String SCDZCLOB) {
        this.SCDZCLOB = SCDZCLOB;
    }

    public String getYPQYSCFW() {
        return YPQYSCFW;
    }

    public void setYPQYSCFW(String YPQYSCFW) {
        this.YPQYSCFW = YPQYSCFW;
    }

    public String getZCDZ() {
        return ZCDZ;
    }

    public void setZCDZ(String ZCDZ) {
        this.ZCDZ = ZCDZ;
    }

    public String getFRDB() {
        return FRDB;
    }

    public void setFRDB(String FRDB) {
        this.FRDB = FRDB;
    }

    public String getQYFZRXM() {
        return QYFZRXM;
    }

    public void setQYFZRXM(String QYFZRXM) {
        this.QYFZRXM = QYFZRXM;
    }

    public String getQYLX() {
        return QYLX;
    }

    public void setQYLX(String QYLX) {
        this.QYLX = QYLX;
    }

    public String getZLFZRXM() {
        return ZLFZRXM;
    }

    public void setZLFZRXM(String ZLFZRXM) {
        this.ZLFZRXM = ZLFZRXM;
    }

    public String getQYFLM() {
        return QYFLM;
    }

    public void setQYFLM(String QYFLM) {
        this.QYFLM = QYFLM;
    }

    public String getFZJG() {
        return FZJG;
    }

    public void setFZJG(String FZJG) {
        this.FZJG = FZJG;
    }

    public String getFZRQ() {
        return FZRQ;
    }

    public void setFZRQ(String FZRQ) {
        this.FZRQ = FZRQ;
    }

    public String getZJZSYXQZZRQ() {
        return ZJZSYXQZZRQ;
    }

    public void setZJZSYXQZZRQ(String ZJZSYXQZZRQ) {
        this.ZJZSYXQZZRQ = ZJZSYXQZZRQ;
    }

    public String getDZSQZZZM() {
        return DZSQZZZM;
    }

    public void setDZSQZZZM(String DZSQZZZM) {
        this.DZSQZZZM = DZSQZZZM;
    }

    public String getYXKZBH() {
        return YXKZBH;
    }

    public void setYXKZBH(String YXKZBH) {
        this.YXKZBH = YXKZBH;
    }

    public String getBGLSJL() {
        return BGLSJL;
    }

    public void setBGLSJL(String BGLSJL) {
        this.BGLSJL = BGLSJL;
    }

    public String getPPZT() {
        return PPZT;
    }

    public void setPPZT(String PPZT) {
        this.PPZT = PPZT;
    }

    public String getZTBZYY() {
        return ZTBZYY;
    }

    public void setZTBZYY(String ZTBZYY) {
        this.ZTBZYY = ZTBZYY;
    }

    public String getZTBZSJ() {
        return ZTBZSJ;
    }

    public void setZTBZSJ(String ZTBZSJ) {
        this.ZTBZSJ = ZTBZSJ;
    }

    public String getID() {
        return ID;
    }

    public void setID(String ID) {
        this.ID = ID;
    }
}
